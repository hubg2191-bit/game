...
export class HeroMesh {
  ...
  buildRig(arch, teamHex) {
    ...
    const W = {
      warlord() {
        ...
        cone(g, 0.12, 0.5, 0xe8e0cc, -0.3, 2.0, 0).rotation.z = 0.7; // рога
        cone(g, 0.12, 0.5, 0xe8e0cc, 0.3, 2.0, 0).rotation.z = -0.7;
        ...
        const flag = new THREE.Mesh(new THREE.PlaneGeometry(0.75, 0.55), teamMat.clone());
        flag.position.set(-0.12, 1.7, -0.1);
        g.add(flag);
      },
      steward() {
      ...
      paladin() {
        ...
        cyl(arm, 0.42, 0.42, 0.08, 0xe8e0cc, 0, 0.35, 0.2, Math.PI / 2, 0, 0); // щит-солнце
        sph(arm, 0.15, 0xe8b400, 0, 0.35, 0.26); // босс-солнце
      ...
      druid() {
        ...
        cone(g, 0.18, 0.45, 0x3f7a3f, 0.5, 2.1, 0.1); // листва
        cone(g, 0.4, 0.6, 0x3a4a2a, 0, 1.88, -0.03); // капюшон
      ...
      barbarian() {
        ...
        box(arm, 0.14, 0.85, 0.1, 0x6b4a2f, 0, 0.3, 0.12); // топор 1
        box(arm, 0.36, 0.26, 0.06, 0x8a8f96, 0, 0.72, 0.12);
        const arm2 = new THREE.Group();
        arm2.position.set(-0.45, 1.2, 0);
        g.add(arm2);
        box(arm2, 0.14, 0.85, 0.1, 0x6b4a2f, 0, 0.3, 0.12); // топор 2
        box(arm2, 0.36, 0.26, 0.06, 0x8a8f96, 0, 0.72, 0.12);
      ...
      rogue() {
        ...
        cone(g, 0.4, 0.6, 0x1a1a22, 0, 1.88, -0.03); // капюшон
        box(g, 0.4, 0.16, 0.1, 0x0a0a0e, 0, 1.58, 0.2); // маска
        box(arm, 0.11, 0.65, 0.05, 0x9aa0a8, 0, 0.2, 0.12); // кинжал
        const arm2 = new THREE.Group();
        arm2.position.set(-0.45, 1.2, 0);
        g.add(arm2);
        box(arm2, 0.11, 0.65, 0.05, 0x9aa0a8, 0, 0.2, 0.12);
        g.userData.arm2 = arm2;
        cloak.material.color.set(0x2a1a3a); // фиолетовый плащ вместо командного
      },
      necromancer() {
        torso.material.color.set(0x14141a);
        cone(g, 0.48, 1.25, 0x14141a, 0, 0.85, 0); // балахон
        cyl(g, 0.04, 0.05, 1.9, 0x3a3028, 0.5, 0.95, 0.1); // череп-посох
        sph(g, 0.18, 0xe8e0cc, 0.5, 1.95, 0.1); // череп
      ...
      mage() {
        ...
        const cry = new THREE.Mesh(new THREE.OctahedronGeometry(0.22), mat(0xff4422, { emissive: 0xff4422, emissiveIntensity: 0.7 }));
        cry.position.set(0.5, 1.95, 0.1); // кристалл
        g.add(cry);
        box(g, 0.25, 0.32, 0.1, 0x6b2a1a, -0.3, 0.9, 0.3); // книга рун
        cone(g, 0.36, 0.5, 0x5a2018, 0, 1.85, -0.03); // капюшон
      },
    };
    (W[arch] || W.warlord)();
    // иконка класса над головой (крупная — героя видно за 1 сек с высоты)
    ...
    const aura = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: team, transparent: true, opacity: 0.45, side: THREE.DoubleSide })
    );
  ...
  sync(state, time, dt, isVisible, teamHexOf) {
    ...
    for (const s of state.squads) {
      ...
      if (s.castT > 0) {
        s.castT -= dt;
        // Q и E читаются издали: Q — рука вверх, E — обе руки + вспышка иконки и ауры
        const isE = s.castSlot === 'e';
        r.arm.rotation.x = -1.8; // замах посохом/оружием
        if (r.group.userData.arm2) r.group.userData.arm2.rotation.x = isE ? -1.8 : 0;
        r.icon.scale.set(isE ? 2.9 : 2.6, isE ? 2.9 : 2.6, 1);
        r.aura.material.opacity = isE ? 0.85 : 0.6;
      } else {
        ...
        r.icon.scale.set(2.2, 2.2, 1);
        r.aura.material.opacity = 0.45;
      }
